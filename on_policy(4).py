...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        self.rollout_buffer = self.rollout_buffer_class(
            self.n_steps,
            spaces.Box(low=np.repeat(self.observation_space.low[None, :], 10, axis=0),high=np.repeat(self.observation_space.high[None, :], 10, axis=0),dtype=self.observation_space.dtype), 
            self.action_space,
            device=self.device,
            gamma=self.gamma,
    ...
    def collect_rollouts(
        ...
        states=deque([np.zeros((1,1,*self.observation_space.shape)) for i in range(10)], maxlen=10)
        new_states=deque([np.zeros((1,1,*self.observation_space.shape)) for i in range(10)], maxlen=10)
        ...
        while n_steps < n_rollout_steps:
            ...
            states.append(np.expand_dims(self._last_obs , axis=1))
            
            with th.no_grad():
                o=np.concatenate(states, axis=1).astype(np.float32)
                
                # Convert to pytorch tensor or to TensorDict
                obs_tensor = obs_as_tensor(o, self.device)  # type: ignore[arg-type]
            ...
            for idx, done in enumerate(dones):
                if (
                    ...
                    fo=states
                    fo.append(np.expand_dims(infos[idx]["terminal_observation"] , axis=(0,1)))
                    fo=np.concatenate(fo, axis=1).astype(np.float32)
                    
                    terminal_obs = obs_as_tensor(fo, self.device)
            ...
            new_states.append(np.expand_dims(self._last_obs , axis=1))
            new_o=np.concatenate(new_states, axis=1).astype(np.float32)
        with th.no_grad():
            # Compute value for the last timestep
            values = self.policy.predict_values(obs_as_tensor(new_o, self.device))  # type: ignore[arg-type]
